...
let displayedKrathongIds = new Set(); // ‼️‼️ เพิ่ม: Set สำหรับเก็บ ID ของกระทงที่แสดงอยู่ เพื่อป้องกันการซ้ำซ้อน ‼️‼️
...
document.addEventListener('DOMContentLoaded', (event) => {
  ...
  function createKrathongElement(kData, isCommunityKrathong = false) {
      ...
      if (displayedKrathongs.length >= MAX_KRATHONGS_ON_SCREEN) {
        return; // ถ้ากระทงเต็มจอแล้ว ไม่ต้องสร้างเพิ่ม
      }
      if (kData.id && displayedKrathongIds.has(kData.id)) {
        return; // ถ้ากระทงนี้แสดงอยู่บนจอแล้ว ไม่ต้องสร้างซ้ำ
      }
      // --- สิ้นสุดการตรวจสอบ ---
      ...
      const perspectiveRatio = (NUM_LANES > 1) ? (laneIndex / (NUM_LANES - 1)) : 0; // ค่าระหว่าง 0 ถึง 1
      const scale = 1.1 - (perspectiveRatio * 0.4); // ขนาดจะอยู่ระหว่าง 1.1 (ใกล้) ถึง 0.7 (ไกล)
      const animationDuration = KRATHONG_ANIMATION_DURATION + (perspectiveRatio * 20); // ความเร็วระหว่าง 100s (ใกล้, เร็ว) ถึง 120s (ไกล, ช้า)
      ...
      krathongWrapper.addEventListener('animationend', () => {
        ...
        // ‼️‼️ เพิ่ม: ลบ ID ออกจาก Set เมื่อกระทงลอยจนสุดจอ ‼️‼️
        if (kData.id) displayedKrathongIds.delete(kData.id);
  ...
  function createMyKrathongElement(kData) {
    ...
    // ‼️‼️ เพิ่ม: เพิ่ม ID เข้าไปใน Set ก่อนที่จะส่งไปสร้าง ‼️‼️
    if (kData.id) {
      displayedKrathongIds.add(kData.id);
    }
